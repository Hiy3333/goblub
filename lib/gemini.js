...
export async function streamGemini(res, opts) {
  ...
  // 창작 페르소나 작업이라 thinking 토큰이 출력 예산을 잠식해 답이 잘리는 걸 방지(flash 계열 thinking off). pro는 사고가 필수라 제외.
  if (!/pro/.test(mdl)) genConfig.thinkingConfig = { thinkingBudget: opts.thinkingBudget == null ? 0 : opts.thinkingBudget };
  const payload = {
