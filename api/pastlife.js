...
- 가장 임팩트 있는 전생 한 장면을 그린다. 배경색 1개를 전체에 깔고, 그 위에 큼직한 flat 도형(원·사각형·타원·삼각형·path) 8~16개로 주인공과 소품의 실루엣을 알아보게 배치. 파스텔 또는 선명한 색.
- 형태가 뭉개지지 않게 도형을 겹쳐 실루엣을 만들고 눈·입 등 잔디테일은 최소화. 텍스트 라벨은 0~1개(한글, font-size 24 이상)만.
- 반드시 문법적으로 완결된 유효 SVG로, 열린 태그를 모두 닫고 마지막에 </svg>로 끝맺어라. script·외부 이미지/폰트 참조·이벤트 속성(on*) 금지. 좌표는 600x400 안에서만.`;
